...
describe("SphericalGeometrySubdivisionModifier", function() {
    it("SubdivideTileBounds", function() {
        ...
        const idxBuffer = new THREE.BufferAttribute(new Float32Array([0, 1, 2, 2, 1, 3]), 1);
        geometry.setIndex(idxBuffer);
        ...
        for (let i = 0; i < idxBuffer.length; i += 3) {
            a.set(
                posAttr.array[idxBuffer.array[i]] * 3,
                posAttr.array[idxBuffer.array[i]] * 3 + 1,
                posAttr.array[idxBuffer.array[i]] * 3 + 2
            );
            b.set(
                posAttr.array[idxBuffer.array[i + 1]] * 3,
                posAttr.array[idxBuffer.array[i + 1]] * 3 + 1,
                posAttr.array[idxBuffer.array[i + 1]] * 3 + 2
            );
            c.set(
                posAttr.array[idxBuffer.array[i + 2]] * 3,
                posAttr.array[idxBuffer.array[i + 2]] * 3 + 1,
                posAttr.array[idxBuffer.array[i + 2]] * 3 + 2
            );
